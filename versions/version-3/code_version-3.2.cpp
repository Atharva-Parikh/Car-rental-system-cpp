...
class Login_user:public Login{
	public:
		...
		void book_luxury();
		//invoices
		void invoice(int,char* name_of_car2);
		void bill(float,int,char* name_of_car);   //void bill(float,int,char*);
		//do work function
		void do_user_work();
		void unbook();
...
void Login_user :: Register()
{
	...
	do{
		...
		val = check_user_existence(username);
...
void Login_user :: book_hatchback()
{
	...
	if(found=='n'){
		...
		cout<<"Character pointer is working "<<name_of_car1<<endl;
		this->invoice(car_to_book,name_of_car1); //this->invoice(car_to_book,name_of_car);
...
void Login_user :: book_sedan()
{
	...
	if(found=='n'){
		...
		//this->invoice(car_to_book,name_of_car);
		//cout<<"Thank You for Booking!"<<endl;
		return;
	}
}
...
void Login_user::unbook()
{
	...
	while(1)
	{
		...
		if(c.get_ID() == id_car && c.booked_status()==1){
			...
			cout<<"This is not Your Car ID.\nPlease enter the correct ID"<<endl;
			found = 'j';
			break;
		}
	}
...
void Login_user::invoice(int carid,char* name_of_car2) //(int carid,char* name_of_car)
{
	int see_car;
	see_car = search_car_hatchback(carid);
	if(see_car==1)
	{
		bill(50,carid,name_of_car2); //bill(50,carid,name_of_car);
		return;
	}
	see_car = search_car_sedan(carid);
	if(see_car==1)
	{
		//bill(75,carid); //bill(75,carid,name_of_car);
		return;
	}
	see_car = search_car_suv(carid);
	if(see_car==1)
	{
		//bill(85,carid); //bill(5,carid,name_of_car);
		return;
	}
	see_car = search_car_luxury(carid);
	if(see_car==1)
	{
		//bill(220,carid); //bill(220,carid,name_of_car);
		return;
	}
}
void Login_user::bill(float rate,int carid,char* name_of_car)  //(float rate,int carid,char* name_of_car)
{
	int c,n,bill_id;
	...
	switch(c)
	{
		case 1:
			...
			cout<<"|"<<setw(30)<<"Car Name"<<"|"<<setw(30)<<name_of_car<<endl;
			cout<<"|"<<setw(30)<<"Numbers of Hours"<<"|"<<setw(30)<<n<<endl;
			cout<<"|"<<setw(30)<<"Advance Pay "<<"|"<<setw(30)<<min_pay<<endl;
			cout<<"|"<<setw(30)<<"Total Pay"<<"|"<<setw(30)<<pay-min_pay<<endl;
...
void Login_user :: do_user_work()
{
	...
	switch(choice)
	{
		case 1:
			int ch;
			int collect;
			//info about cars in short
			cout<<"Booking Point\n 1.Hatchback\n 2.Sedan\n 3.Suv\n 4.Luxury\n Your Choice: ";
			cin>>ch;
			switch(ch)
			{
				case 1:
					collect = this->show_hatchback();
					if(collect == 0)
						this->book_hatchback();	
					break;
				case 2:
					collect = this->show_sedan();
					if(collect == 0)
						this->book_sedan();
					break;
				case 3:
					collect = this->show_suv();
					if(collect == 0)
						this->book_suv();
					break;
				case 4:
					collect = this->show_luxury();
					if(collect == 0)
						this->book_suv();
					break;
				default:
...
void Login_user :: login(char user_n[], char pass[])
{
	...
	while(fin>>username>>password)
	{
		if(strcmp(user_n,this->username)==0 && strcmp(pass,this->password)==0)
		{
			...
			round = 1;
			cout<<"Found value (code line 1194): "<<round<<endl;
			do_user_work();
		}
		else if(strcmp(user_n,this->username)==0)
		{
